    ...
    <div class="panel-header" style="flex-wrap: wrap; gap: 0.75rem;">
        ...
        <div style="display: flex; align-items: center; gap: 0.75rem; flex-wrap: wrap;">
            ...
            <form class="filter-actions" action="{{ route('admin.gaji.slip') }}" method="GET" style="display:flex;gap:0.5rem;align-items:center;">
                ...
                <input type="text" id="searchInput" list="karyawanList" class="form-control" placeholder="Ketik nama atau NIK..." style="padding:0.35rem 0.75rem;font-size:0.85rem; min-width:200px;" value="{{ isset($penggajian) ? strtoupper($penggajian->user->name) . ' (' . ($penggajian->user->nik ?? '-') . ')' : '' }}">
                ...
                <input type="hidden" name="user_id" id="hiddenUserId" value="{{ isset($penggajian) ? $penggajian->user_id : '' }}">
                <label style="font-size: 0.82rem; font-weight: 600; color: var(--text-secondary); white-space:nowrap; margin-left: 0.5rem;">Periode:</label>
                <input type="month" name="bulan_tahun" class="form-control" style="padding:0.35rem 0.75rem;font-size:0.85rem; width:150px;" value="{{ request('bulan_tahun', date('Y-m')) }}" onchange="this.form.submit()">
                ...
                <table class="table-report">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><input type="number" name="absensi[{{ $abs->id }}][jam_lembur]" value="{{ $realJamLembur ?: 0 }}" class="input-jam" data-rate="{{ $penggajian->user->uang_lembur_per_jam }}" style="width: 50px; text-align: center; border: 1px solid var(--border-color); border-radius: 4px; padding: 0.3rem;" min="0" step="1"></td>
